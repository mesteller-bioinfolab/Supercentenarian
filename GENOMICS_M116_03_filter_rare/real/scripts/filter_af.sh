...
filter_vep -i ${INPUTS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.vep2 -o ${OUTPUTS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.filtered --force_overwrite --filter "EUR_AF < 0.015 or gnomADg_NFE_AF < 0.015 or gnomADe_NFE_AF < 0.015 or (not gnomADg_NFE_AF and not gnomADe_NFE_AF and not EUR_AF)" --format vcf --vcf_info_field ANN --only_matched


#
#bcftools view --types snps ${INPUTS_DIR}/${TISSUED}/${IDD}/${IDD}.${CHR}.vcf.gz > ${TMPS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.snvs
#bcftools filter -e 'ANN/EUR_AF>0.015' ${TMPS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.snvs > ${OUTPUTS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.filtered
#gzip ${OUTPUTS_DIR}/${TISSUED}/${IDD}/${TISSUED}.${IDD}.${CHR}.filtered
